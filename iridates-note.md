...
[Dirac line node with symmetry analysis](/public/iridatesnotes/Dirac-line-symmetry-analysis-J-half-states.pdf)
...
[inversion symmetry corrected](/public/iridatesnotes/inversion-symmetry-corrected.pdf)
...
[\\( \sqrt{2} \times \sqrt{2} \\) Hamiltonian](/public/iridatesnotes/sqrt2unitecell-Hamiltonian.pdf)
...
각도에 따른 Slater-Koster parameter를 계산하기 위해서 다음 [논문10-Konschuh-TB-SOC-graphene-PRB.pdf](/public/iridatesnotes/10-Konschuh-TB-SOC-graphene-PRB.pdf) 으로 부터 영감을 받아서 [mathematica file d-orbital-rotation-translation_160922.nb](/public/iridatesnotes/)을 만들어 보았다.

여기서 다음 [노트](/public/iridatesnotes/slater-Koster-parameter-d-orbital-rotation.pdf)와 같이 Slater-Koster parameter를 만들면 될거 같다.

그리고 논문 검색중 Kane 그룹에서 spin-orbit semimetal에 대한 논문을 찾았다. [16-Wieder](/public/iridatesnotes/16-Wieder-spin-orbit-semimetals-arXiv.pdf)
...
[rotation Slater-Koster parameter](/public/iridatesnotes/Rotation-slater-Koster.pdf) (p-orbital 로 연습)
...
[rotation Slater-Koster parameter2](/public/iridatesnotes/Rotation-slater-Koster2.pdf) ( \\( E_{zx,zx}, E_{yz, yz} \\) 계산, 그리고 마지막에 \\( E_{xy, xy} \\) 값을 예상해보았으나 밑에서 잘못됐음을 알고 아래에 수정했음. )

내일 할일은 \\( d_{xy} \\) orbital 끼리의 hopping 이 과연 위에 적은 대로 될것인가를 검증하는 일이다.

이계산할때 사용한 [mathematica file_SK-parameter-d_xy-2016SEP26.nb](/public/iridatesnotes/SK-parameter-d_xy-2016SEP26.nb)

논문 검색을 하다가 Colorado에서 Sr_2IrO_4 에 대한 실험 논문을 찾았다. [paper](/public/iridatesnotes/15-Yang-High-energy-electronic-excitation-PRB.pdf)


2016/09/27 (화)

이제 local rotation 과 공간 이동이 있을 때 \\( E_{yz, yz}, E_{zx,zx}, E_{xy,xy} \\) Slater-Koster parameter를 구하는 것이 끝났다. [관련 note를 첨부한다.](/public/iridatesnotes/Rotation-slater-Koster3.pdf) ( \\( E_{zx,zx}, E_{yz, yz}, E_{xy, xy} \\) 를 정리해놓았음. )

이제 \\( E_{yz, zx}, E_{zx, yz} \\) 를 살펴볼 차례이다.
--> done!

[ \\( E_{yz, zx}, E_{zx, yz}  \\) note를 첨부한다.](/public/iridatesnotes/Rotation-slater-Koster4.pdf)
...
이계산할때 사용한 [mathematica file_SK-parameter-d_xy-2016SEP27.nb](/public/iridatesnotes/SK-parameter-d_xy-2016SEP27.nb)

이제 tight-binding model를 만들 차례이다.

[d_{yz} x축에 대해서 하나 계산해봄](/public/iridatesnotes/tight-binding1.pdf)
...
먼저 그전에 [glide mirror operator applies to J=1/2 state A](/public/iridatesnotes/glide-symmetry-WannierFT.pdf)
10/06 (목)
glide mirror operator \\( G_y \\) 를 J=1/2 state 에 적용하고, \\( G_x \\) [를 d_{yz} A,B 에 적용해보았다.](/public/iridatesnotes/glide-symmetry-WannierFT2.pdf)그럼 unit cell 를 어떻게 잡느냐에 따라서 extra phase가 다를 것인데, unitary transform 하고 나면 서로다른 두가의 경우만 남게 된다. [extra phase unit cell dependence](/public/iridatesnotes/glide-symmetry-WannierFT3.pdf)
먼저 나머지 오비탈 \\( d_{zx}, d_{xy} \\)에 대해서 G_x를 적용해 보고, 그 다음 \\( G_y \\) 를 [J=1/2 상태에 적용해보았다.](/public/iridatesnotes/glide-symmetry-WannierFT4.pdf)
주요결과는 unitary 변환된 Hamiltonian의 에너지값과 degeneracy는 nonsymmorphic operator 에 의해 변하지 않는다는 것이다. [0000_iridate_band-2D-Sr214-Gx.nb](/public/iridatesnotes/0000_iridate_band-2D-Sr214-Gx.nb)
